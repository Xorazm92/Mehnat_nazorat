...
export class SalaryUpdate {
  ...
  async salaryMenu(@Ctx() ctx: ContextType): Promise<void> {
  ...
  async getMySalary(@Ctx() ctx: ContextType): Promise<void> {
    ...
    if (
      ...
      if (history.length === 0) {
        await ctx.reply(SALARY_MESSAGES.NO_SALARY_DATA);
        return;
      }
