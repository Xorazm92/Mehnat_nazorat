...
export class CalculateSalaryScene {
    ...
    async onEnter(@Ctx() ctx: ContextType) {
        ...
        await ctx.reply(
            "Maosh hisoblash uchun xodimni tanlang:",
            {
                reply_markup: {
    ...
    async onCallback(@Ctx() ctx: ContextType) {
        ...
        if (data.startsWith('salary_emp_')) {
            ...
            await ctx.reply("Ushbu oy uchun maosh miqdorini kiriting (bonus va jarimalarni keyingi qadamda kiritasiz):", BACK_KEYBOARD);
        }
    }
    ...
    async onText(@Ctx() ctx: ContextType) {
        ...
        switch (currentStep) {
            ...
            case 'penalty':
                ...
                // Confirm
                const total = ctx.session.salaryBase + ctx.session.salaryBonus - penalty;
                const msg = `💰 Hisoblash yakuni:\n\n` +
    ...
    async confirm(@Ctx() ctx: ContextType) {
        ...
        try {
            ...
            const salary = await this.salaryService.getSalaryByMonth(ctx.session.salaryEmployeeId, monthStr);
            ...
            await ctx.reply("✅ Maosh muvaffaqiyatli saqlandi!", MANAGER_MAIN_KEYBOARD);
            await ctx.scene.leave();
        } catch (e) {
            console.error(e);
