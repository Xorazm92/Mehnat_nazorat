...
export class GetAppealsText {
  ...
  async onEnter(@Ctx() ctx: ContextType) {
    const existing: any = await this.cache.get(`appeal${ctx.from.id}`);
    if (!existing) {
  ...
  async onText(@Ctx() ctx: ContextType) {
    const text = ((ctx.update as any).message.text ?? '').trim();
    if (!text) {
      await ctx.reply(
        `⚠️ Murojat matni bo'sh bo'lishi mumkin emas. Qaytadan kiriting:`,
      );
      return;
    }
