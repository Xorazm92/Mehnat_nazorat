...
import { ButtonsService } from 'src/api/bot/buttons/buttons.service';
...
export class ManageUsersActions {
  ...
  private async requireAdmin(ctx: ContextType): Promise<User | null> {
    ...
    if (!user) {
      try {
        await ctx.answerCbQuery(MESSAGES.ERROR, { show_alert: true });
      } catch {
    ...
    if (user.status !== UserStatus.ACTIVE || user.role !== UserRole.MANAGER) {
      try {
        await ctx.answerCbQuery(MESSAGES.NO_PERMISSION, { show_alert: true });
      } catch {
