...
export class StatisticsUpdate {
  ...
  async statisticsMenu(@Ctx() ctx: ContextType): Promise<void> {
  ...
  async myStatistics(@Ctx() ctx: ContextType): Promise<void> {
  ...
  async getStats(@Ctx() ctx: ContextType): Promise<void> {
    const user = await this.getUser(ctx);
    if (!user) {
      await ctx.reply(MESSAGES.ERROR);
      return;
  ...
  @Hears('🏆 Reytinglar')
  async getRankings(@Ctx() ctx: ContextType): Promise<void> {
    const topPerformers = await this.statisticsService.getTopPerformers(
      PeriodType.MONTHLY,
    );
    if (topPerformers.length === 0) {
      await ctx.reply("Reyting ma'lumotlari mavjud emas.");
      return;
    }
  ...
  async employeeSpecificStats(@Ctx() ctx: ContextType): Promise<void> {
